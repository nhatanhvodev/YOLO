...
# Base directories for relative path management
...
for _d in [DIR_XE_MAY_VP, DIR_OTO_VP, DIR_BIEN_BAN_M, DIR_BIEN_BAN_CTB]:
...
def imageMotorViolate(frame, r1, r2, c1, c2, stt):
    ...
    name_file = f"{stt}.jpg"
    output_path = os.path.join(DIR_XE_MAY_VP, name_file)
...
def imageCTBViolate(frame, r1, r2, c1, c2, stt):
    ...
    cropped_region = frame[r1:r2, c1:c2]
    name_file = f"{stt}.jpg"
    output_path = os.path.join(DIR_OTO_VP, name_file)
    cv2.imwrite(output_path, cropped_region)
    return output_path
...
if __name__ == '__main__':
    cap = cv2.VideoCapture(os.path.join(BASE_DIR, 'Videos', 'main.mp4'))
